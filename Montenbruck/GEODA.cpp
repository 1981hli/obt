...
struct Station {
  Geodetic Geod;                                                                ///< @details Geodetic coordinates
...
/// Purpose:
///
/// Reads GEODA setup parameters from file \f$\sqrt{(x_2-x_1)^2+(y_2-y_1)^2}\f$.
///
//-----------------------------------------------------------------------------
...
void GetSetup ( ifstream& inp,
  ...
  Sta2.ang.sig_noise*=Rad; Sta2.ang.sig_bias*=Rad;
                                                                                ...
                                                                                // (estimation parameters first,
                                                                                // consider parameters last)

  n_est =  6;                                                                   // Estimation parameter count 
                                                                                // (default: state vector only)
...
int main(int argc, char* argv[]) {
  ...
  C = Diag(Var_b.slice(n_est-n_orb,n_bias-1)); 
                                                                                ...
                                                                                // Partial derivatives of inertial 
                                                                                // w.r.t. rotating state vector
  ...
  Lsq.Init();
                                                                                ...
                                                                                // Process partials of measurements 
                                                                                // from both stations
  ...
  for (i_Sta=1;i_Sta<=2; i_Sta++) {
    ...
    if (Sta.ang.step>0.0) for(;;) {
      ...
      Lsq.Accumulate ( Stack(dhdy,dhdb), 0.0, Sta.ang.sig_noise );
                                                                                ...
                                                                                // Increment time of measurement and 
                                                                                // exit if past end of data arc
    ...
    if (Sta.rng.step>0.0) for(;;) {
      ...
      Lsq.Accumulate ( Stack(dhdy,dhdb), 0.0, Sta.rng.sig_noise );
                                                                                // Increment time of measurements and 
                                                                                // exit if past end of data arc
      t += Sta.rng.step;
      if (t>T_track) break;
    };
...
*/
  ...
  for(;;) {
    ...
    cout << fixed 
         ...
         << endl;
                                                                                // Increment time and exit if past end
                                                                                // of prediction arc
    t += 3.0;
    if (t>T_pred) break;
  };
