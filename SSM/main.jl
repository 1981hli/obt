...
function gravity_Newton_by1(test::Body,source::Body)
  dx=source.x-test.x
  gravity=G*test.mass*source.mass/sqrt(sum(dx.^2))^3*dx
end
...
function gravity_Newton_byall(step::Step,testnum::Int)
  gravity=[0.,0.,0.]
  for i in 1:TotalBody
    if i==testnum continue end
    gravity=gravity+gravity_Newton_by1(step.body[testnum],step.body[i])
  end
  gravity
end
...
# function forcePPN(step::Step,A::Int)
  ...
  # function forcePPNby1(step::Step,A::Int,B::Int)
    ...
    # T2 =-2*(beta+gamma)/c^2*
        # sum( G*m(i)/norm_r(A,i) for i=[1:TotalBody;] if i!=A )
    # T3 =-(2*beta-1)/c^2*
        # sum( G*m(i)/norm_r(B,i) for i=[1:TotalBody;] if i!=B )
    # T4 =gamma*(v(A)/c)^2
    # T5 =(1+gamma)*(v(B)/c)^2
    # T6 =-2(1+gamma)/c^2*dot(v(A),v(B))
    # T7 =-3/(2*c^2)*(dot(r(B,A),v(B))/norm_r(A,B))^2
    # T8 =1/(2*c^2)*dot(r(A,B),a(B))
    # T9 =1/c^2*G*m(B)/norm_r(A,B)^3*
        # dot(r(B,A),(2+2*gamma)*v(A)-(1+2*gamma)*v(B))*v(B,A)
    # T10=(3+4*gamma)/(2*c^2)*G*m(B)/r(A,B)*a(B)
    # acceleration=T1*(1+T2+T3+T4+T5+T6+T7+T8)+T9+T10
  # end

  # sum(forcePPNby1(step,A,B) for B=[1:TotalBody;] if B!=A)
# end
...
function rk_rk4(thestep::Step,dt::Float64)
  ...
  function rk_dstep(thestep::Step,dt::Float64)
    ...
    tmp.time=dt
    for i in 1:TotalBody
      force=gravity_Newton_byall(thestep,i)
      tmp.body[i].x=thestep.body[i].v*dt # dx=v*dt
      tmp.body[i].v=1/thestep.body[i].mass*force*dt # dv=a*dt
    end
    tmp
  end
